...
describe('report service unit tests', () => {
  ...
  test('getReportsAssigned returns empty when no assignments', async () => {
    mockQuery.mockResolvedValueOnce({ rows: [] });
    const res = await reportsService.getReportsAssigned(999);
    expect(res).toEqual([]);
  });

  test('getReportsAssigned handles multiple assignments and filters by operator_id', async () => {
    const rows = [
      {
        report_id: 10,
        title: 'Report10',
        ...
        updated_at: new Date(),
        citizen_id: 16,
        citizen_username: 'u16',
        citizen_first_name: 'F16',
        citizen_last_name: 'L16',
        category_id: 5,
        category_name: 'c5',
        office_id: 5,
        office_name: 'o5',
        status_id: 2,
        status_name: 'Approved',
        assigned_to_operator_id: 100,
        assigned_to_external_id: null,
        operator_username: 'op100',
        operator_email: 'op100@example.com',
        company_name: 'CompA',
        photos: [],
      },
      {
        ...
        updated_at: new Date(),
        citizen_id: 17,
        citizen_username: 'u17',
        citizen_first_name: 'F17',
        citizen_last_name: 'L17',
        category_id: 6,
        category_name: 'c6',
        office_id: 6,
        office_name: 'o6',
    ...
    const res = await reportsService.getReportsAssigned(100);
    expect(res).toHaveLength(2);
    expect(res[0].assigned_to_operator.id).toBe(100);
    expect(res[1].assigned_to_operator.id).toBe(100);
  });