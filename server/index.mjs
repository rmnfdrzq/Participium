...
// POST /api/reports/:id/internal-comments (operators only)
app.post("/api/reports/:id/internal-comments", async (req, res) => {
  try {
    if (!req.isAuthenticated())
      return res.status(401).json({ error: "Not authenticated" });
...
// GET /api/reports/:id/internal-comments (operators only)
app.get("/api/reports/:id/internal-comments", async (req, res) => {
  try {
    if (!req.isAuthenticated())
      return res.status(401).json({ error: "Not authenticated" });
    ...
    console.error(err);
    res.status(503).json({ error: "Database error during comment retrieval" });
  }
});

// POST /api/reports/:id/messages (citizen - operator communication)
app.post("/api/reports/:id/messages", async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(503).json({ error: "Database error during message creation" });
  }
});

// GET /api/reports/:id/messages (citizen - operator communication)
app.get("/api/reports/:id/messages", async (req, res) => {
  try {
    ...
    console.error(err);
    res.status(503).json({ error: "Database error during message retrieval" });
  }
});
